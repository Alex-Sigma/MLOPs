...
RESULTS_DIR = Path("results") / "evaluate"
...
@click.command()
@click.option("--test_data_dir", default=TEST_DATA_DIR, type=Path)
@click.option("--model_dir", default=MODEL_DIR, type=Path)
@click.option("--results_dir", default=RESULTS_DIR, type=Path)
@click.option("--n_samples_to_save", default=N_SAMPLES_TO_SAVE, type=int)
def evaluate(test_data_dir, model_dir, results_dir, n_samples_to_save):
    results_dir.mkdir(exist_ok=True, parents=True)
    model_fpath = model_dir / "model.pkl"
    learn = load_learner(model_fpath, cpu=False)
    test_img_fpaths = sorted(get_files(test_data_dir, extensions=".jpg"))
    test_dl = learn.dls.test_dl(test_img_fpaths)
    preds, _ = learn.get_preds(dl=test_dl)
    masks_pred = np.array(preds[:, 1, :] > 0.5, dtype=np.uint8)
    test_mask_fpaths = [
        get_mask_path(fpath, test_data_dir) for fpath in test_img_fpaths
    ]
    ...
    for ii in range(len(masks_true)):
        ...
        if ii < n_samples_to_save:
            stacked_image = get_overlay_image(test_img_fpaths[ii], mask_true, mask_pred)
            stacked_image = stacked_image.resize((512, 256))
            stacked_image.save(f"{results_dir}/{Path(test_img_fpaths[ii]).stem}.png")

    logger.info(f"Dice Multi: {dice_multi}")
